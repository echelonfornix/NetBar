...
- Hunt Device mode scans quickly for a few minutes while you turn a mystery device off and on, then highlights likely `OFF?` and `RESTART` candidates so you can rename the thing you just power-cycled.
...
- Hunt Device works best for devices that answer local probes. Some devices block ping, and stale ARP entries can still keep them visible as hints rather than proof.
