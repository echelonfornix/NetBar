...
Use `Hunt Device...` when you deliberately want to identify something. Start the hunt, leave the device on for the first scan, turn the mystery device off for at least 10 seconds, wait for NetBar to mark a likely `NO REPLY?` candidate, turn it back on, then rename the `RESTART` candidate. Hunt mode fast-checks known devices every 4 seconds for 5 minutes.
...
Use `Clear No Reply Mark` to hide a `NO REPLY?` tag after you have checked it. The tag can appear again after the device answers and later stops replying.
